...
- (void)sd_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletionBlock)completedBlock {
    ...
    if (url) {
        ...
        id <QSSDWebImageOperation> operation = [QSSDWebImageManager.sharedManager downloadImageWithURL:url options:options progress:progressBlock completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            if (!wself) return;
...
- (void)setAnimationImagesWithURLs:(NSArray *)arrayOfURLs {
    ...
    for (NSURL *logoImageURL in arrayOfURLs) {
        id <QSSDWebImageOperation> operation = [QSSDWebImageManager.sharedManager downloadImageWithURL:logoImageURL options:0 progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            if (!wself) return;
            dispatch_main_sync_safe(^{
...
- (void)cancelCurrentImageLoad {
    ...
    id <QSSDWebImageOperation> operation = objc_getAssociatedObject(self, &operationKey);
    if (operation) {
        [operation cancel];
...
- (void)cancelCurrentArrayLoad {
    ...
    for (id <QSSDWebImageOperation> operation in operations) {
        if (operation) {
            [operation cancel];
        }
    }
